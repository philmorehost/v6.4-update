...
<!DOCTYPE html>
<head>
    <title>Payment Link | <?php echo $get_all_site_details["site_title"]; ?></title>
    <meta charset="UTF-8" />
    ...
    <section class="section dashboard">
      <div class="col-12">
        ...
        <?php
            ...
            $get_user_transaction_details = mysqli_query($connection_server, "SELECT * FROM sas_payment_links WHERE vendor_id='".$get_logged_user_details["vendor_id"]."' && username='".$_SESSION["user_session"]."' $search_statement ORDER BY date DESC LIMIT 20 $offset_statement");
            
        ?>
        <div class="card info-card px-5 py-5">
            <div class="row">
                <form method="get" action="PaymentLink.php" class="mt-1">
...
<div style="user-select: auto; cursor: grab;" class="overflow-auto">
    <table style="" class="table table-responsive table-striped table-bordered" title="Horizontal Scroll: Shift + Mouse Scroll Button">
        <thead class="thead-dark">
          <tr>
              <th>S/N</th><th>Reference</th><th>Currency</th><th>Amount</th><th>Min Qty</th><th>Max Qty</th><th style="">Description</th><th>Status</th><th>Date</th>
          </tr>
        </thead>
        <tbody>
                    <?php
                    if(mysqli_num_rows($get_user_transaction_details) >= 1){
                    	while($user_transaction = mysqli_fetch_assoc($get_user_transaction_details)){
                    		...
                    		echo 
                    		'<tr>
                    			<td>'.$countTransaction.'</td><td style="user-select: auto;">'.$user_transaction["reference"].'</td><td>'.$user_transaction["currency"].'</td><td>'.toDecimal($user_transaction["amount"], 2).'</td><td>'.$user_transaction["min_qty"].'</td><td>'.$user_transaction["max_qty"].'</td><td>'.$user_transaction["description"].'</td><td>'.tranStatus($user_transaction["status"]).'</td><td>'.formDate($user_transaction["date"]).'</td>
                    		</tr>';
                    	}
                    }
                    ?>
            ...
            <div class="mt-2 justify-content-between justify-items-center">
                ...
                <a href="PaymentLink.php?<?php echo $search_parameter; ?>page=<?php echo (trim(strip_tags($_GET["page"])) - 1); ?>">
                ...
                ?>
                <a href="PaymentLink.php?<?php echo $search_parameter; ?>page=<?php echo $trans_next; ?>">
