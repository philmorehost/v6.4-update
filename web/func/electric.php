        ...
        if ($action_function == 1) {
            if (!empty(userBalance(1)) && is_numeric(userBalance(1)) && (userBalance(1) > 0)) {
                if (userBalance(1) >= $amount && !empty($amount) && is_numeric($amount) && !empty($epp) && !empty($type)) {
                    ...
                    $get_item_status_details = mysqli_query_and_fetch_array($connection_server, "SELECT * FROM " . $electric_type_table_name_arrays[$epp] . " WHERE vendor_id='" . $get_logged_user_details["vendor_id"] . "' && product_name='$epp'");
                                ...
                                if (!empty($api_detail["api_key"])) {
                                    if ($api_detail["status"] == 1) {
                                        ...
                                        if ($account_level_table_name_arrays[$get_logged_user_details["account_level"]] == true) {
                                            ...
                                            $product_status_table = mysqli_query_and_fetch_array($connection_server, "SELECT * FROM $electric_type_table_name WHERE vendor_id='" . $get_logged_user_details["vendor_id"] . "' && product_name='" . $product_name . "' LIMIT 1");
                                            ...
                                            if ((userBalance(1) >= $amount) && !empty($amount) && is_numeric($amount)) {
                                                if (($product_table["status"] == 1) && ($product_status_table["status"] == 1)) {
                                                    if (productIDBlockChecker($meter_number) == "success") {
                                                        $check_limit = productIDPurchaseChecker($meter_number, "electric");
                                                        if ($check_limit == "success") {
                                                            ...
                                                            if ($debit_user === "success") {
                                                                ...
                                                                if (in_array($api_response, array("successful"))) {
                                                                    ...
                                                                    $json_response_array = array("ref" => $reference, "status" => "success", "meter_number" => $api_response_meter_number, "token" => $api_response_token, "token_unit" => $api_response_token_unit, "desc" => "Transaction Successful. " . $bonus_message, "response_desc" => $api_response_description);
                                                            ...
                                                            if($check_limit == "limit_exceeded"){
                                                                ...
                                                                $json_response_array = array("status" => "failed", "desc" => "[SECURITY_ALERT] Daily Limit Exceeded For This Meter Number: " . $meter_number . ", Contact Admin for Support");
                                                                $json_response_encode = json_encode($json_response_array, true);
                                                            }else{
