...
if (($select_vendor_table == true) && ($select_vendor_table["website_url"] == $_SERVER["HTTP_HOST"]) && ($select_vendor_table["status"] == 1)) {
	...
	$username_signed_in = 0;
	$api_key_sanitized = mysqli_real_escape_string($connection_server, trim(strip_tags($get_api_post_info["api_key"])));
	if(isset($_SESSION["user_session"])){
	  $username_signed_in = 1;
	}
	$get_user_detail_via_apikey = mysqli_query($connection_server, "SELECT * FROM sas_users WHERE vendor_id='" . $get_vendor_details["id"] . "' && (api_key='" . $api_key_sanitized . "' OR username='".$_SESSION["user_session"]."') LIMIT 1");
