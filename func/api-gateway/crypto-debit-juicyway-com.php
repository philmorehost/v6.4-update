...
if (in_array($source_currency, array_keys($crypto_service_provider_alter_code))) {
	...
	$curl_postfields_data = json_encode(array("amount" => $amount_to_convert, "description" => "Customer payout", "type" => "debit", "wallet_id" => $get_wallet_currency_details["api_wallet_id"]), true);
	curl_setopt($curl_request, CURLOPT_POSTFIELDS, $curl_postfields_data);
	$curl_result = curl_exec($curl_request);
	$curl_json_result = json_decode($curl_result, true);

	fwrite(fopen("ajuicydebitwallet.txt", "a++"), $curl_postfields_data . "\n" . $curl_result . "\n\n");
