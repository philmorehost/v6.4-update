...
if (in_array($product_name, array_keys($card_service_provider_alter_code))) {
	...
	if (in_array($quantity, array_keys($web_virtual_card_size_array))) {
		...
		fwrite(fopen("sudo-nairacard.txt", "a++"), $curl_sudo_create_account_holder_postfields_data . "\n" . $curl_sudo_create_account_holder_result . "\n\n\n");
		...
		if ($curl_sudo_create_account_holder_json_result["statusCode"] == 200 && $curl_sudo_create_account_holder_json_result["data"]["status"] == "active") {
			...
			$funding_id = array("mastercard" => "", "verve" => "690eeec21fbdc03dc00945d7", "visa" => "");
			$debit_id = array("mastercard" => "", "verve" => "690eef7b1fbdc03dc00946a0", "visa" => "");
			$curl_postfields_data = json_encode(array("customerId" => $curl_sudo_create_account_holder_json_result["data"]["_id"], "fundingSourceId" => $funding_id[$product_name], "debitAccountId" => $debit_id[$product_name], "type" => "virtual", "brand" => $card_service_provider_alter_code[$product_name], "currency" => "NGN", "issuerCountry" => "NGA", "status" => "active", "amount" => 3, "spendingControls" => array("spendingLimits" => array(array("interval" => "monthly", "amount" => 100000)), "channels" => array("atm" => true, "pos" => true, "web" => true, "mobile" => true), "allowedCategories" => [], "blockedCategories" => [])), true);
			...
			fwrite(fopen("sudo-nairacard-card.txt", "a++"), $curl_postfields_data . "\n" . $curl_result . "\n\n\n");
