...
$curl_url = "https://api.ufitpay.com/v1/change_card_pin";
...
$curl_postfields_data = json_encode(array("id" => $virtual_card_detail["card_id"]), true);
...
if (in_array($curl_json_result["status"], array("success"))) {
	mysqli_query($connection_server, "UPDATE sas_virtualcard_purchaseds SET card_pin = '" . $curl_json_result["data"]["new_pin"] . "' WHERE vendor_id = '" . $get_logged_user_details["vendor_id"] . "' && reference= '$card_ref' && username='" . $get_logged_user_details["username"] . "'");
