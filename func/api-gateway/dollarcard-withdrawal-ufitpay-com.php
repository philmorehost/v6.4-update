...
$curl_url = "https://api.ufitpay.com/v1/withdraw_virtual_card_balance";
...
$curl_postfields_data = json_encode(array("id" => $virtual_card_detail["card_id"], "amount" => $amount, "withdrawal_currency" => "USD"), true);
curl_setopt($curl_request, CURLOPT_POSTFIELDS, $curl_postfields_data);
$curl_result = curl_exec($curl_request);
$curl_json_result = json_decode($curl_result, true);
curl_close($curl_request);

// $curl_json_result = '{ "resource": "withdraw_virtual_card_balance", "status":"success", "data": { "id":"GupJFog3iyv32fsu22uc282", "balance": "16000.00" } }';
...
if (in_array($curl_json_result["status"], array("success"))) {
	...
	$api_response_description = "Card Withdrawal Successful";
...
if (!in_array($curl_json_result["status"], array("success"))) {
	$api_response = "failed";
	$api_response_text = $curl_json_result["status"];
	$api_response_description = "Card Withdrawal Failed";
