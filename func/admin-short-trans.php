                ...
                if(mysqli_num_rows($get_user_transaction_details) >= 1){
                    while($user_transaction = mysqli_fetch_assoc($get_user_transaction_details)){
                        if(!empty($user_transaction["api_id"]) && !empty($user_transaction["product_id"])){
                            $get_user_product_details = mysqli_query_and_fetch_array($connection_server, "SELECT * FROM sas_products WHERE vendor_id='".$get_logged_admin_details["id"]."' && id='".$user_transaction["product_id"]."' LIMIT 1");
                            $get_user_api_details = mysqli_query_and_fetch_array($connection_server, "SELECT * FROM sas_apis WHERE vendor_id='".$get_logged_admin_details["id"]."' && id='".$user_transaction["api_id"]."' LIMIT 1");
