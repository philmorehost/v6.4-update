<?php session_start();
	...
	if(($select_vendor_table == true) && ($select_vendor_table["website_url"] == $_SERVER["HTTP_HOST"]) && ($select_vendor_table["status"] == 1)){
		$paystack_keys = mysqli_fetch_assoc(mysqli_query($connection_server,"SELECT * FROM sas_payment_gateways WHERE vendor_id='".$select_vendor_table["id"]."' && gateway_name='paystack'"));
			...
			if(mysqli_num_rows($check_vendor_user_exists) == 1){
				...
				if(($paystack_verify_transaction["data"]["status"] == "success") && ($catch_incoming_request["data"]["status"] == "success")){
					if(mysqli_num_rows($select_transaction_history) == 0){
						chargeUser("credit", $_SESSION["user_session"], "Wallet Credit", $reference, $transaction_id, $amount_paid, $amount_deposited, "Paystack Wallet Credit - ".str_replace("_"," ",$payment_method), strtoupper("WEB"), $_SERVER["HTTP_HOST"], "1");
