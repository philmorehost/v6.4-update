    ...
    if (($select_vendor_table == true) && ($select_vendor_table["website_url"] == $_SERVER["HTTP_HOST"]) && ($select_vendor_table["status"] == 1)) {
        ...
        if (mysqli_num_rows($checkuser) == 1) {
            ...
            if (mysqli_num_rows($checkuser_pass) == 1) {
                ...
                $select_virtual_card = mysqli_query($connection_server, "SELECT * FROM sas_virtualcard_purchaseds WHERE vendor_id='" . $select_vendor_table["id"] . "' && username='" . $get_user_info["username"] . "' LIMIT 1");
                if (mysqli_num_rows($select_virtual_card) == 1) {
                    $get_virtual_card_detail = mysqli_fetch_array($select_virtual_card);

                    $api_file_url = $_SERVER["DOCUMENT_ROOT"] . "/web/api/view-card.php";
                    if (file_exists($api_file_url)) {
                        $api_post_info_from_app = array("api_key" => $get_user_info["api_key"], "card_ref" => $get_virtual_card_detail["reference"]);
                    ...
                    $status_msg = "Error: Card Not Exists";
                }
            } else {
        ...
        $app_json = json_encode(array("json-status" => "success", "status" => $status_update, "status-msg" => $status_msg, "reference" => $get_virtual_card_detail["reference"], "date" => $get_virtual_card_detail["date"]), true);
    } else {
