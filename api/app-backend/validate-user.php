...
if (json_last_error() === JSON_ERROR_NONE) {
    ...
    if (($select_vendor_table == true) && ($select_vendor_table["website_url"] == $_SERVER["HTTP_HOST"]) && ($select_vendor_table["status"] == 1)) {
        ...
        $type = mysqli_real_escape_string($connection_server, trim(strip_tags($decode_post_request["type"])));
        $id_number = mysqli_real_escape_string($connection_server, trim(strip_tags($decode_post_request["id-number"])));
        ...
        if (
            ...
            if (mysqli_num_rows($checkuser) == 1) {
                ...
                if (mysqli_num_rows($checkuser_pass) == 1) {
                    ...
                    $update_validation = mysqli_query($connection_server, "UPDATE sas_users SET " . strtolower(trim($type)) . " = '" . $id_number . "' WHERE vendor_id = '" . $select_vendor_table["id"] . "' AND (username = '" . $username . "' OR email='" . $username . "')");
                    ...
                    if ($update_validation === true) {
                        ...
                        $select_beewave_gateway = mysqli_query($connection_server, "SELECT * FROM sas_payment_gateways WHERE vendor_id = '" . $select_vendor_table["id"] . "' AND gateway_name='beewave' AND `status`='1' LIMIT 1");
                        if (mysqli_num_rows($select_beewave_gateway) == 1) {
                            ...
                            $beewave_create_reserve_account_array = array("email" => $user_beewave_account_reference, "name" => trim($get_user_detail["firstname"] . " " . $get_user_detail["lastname"]), "phone_number" => $get_user_detail["phone_number"], "access_key" => $get_beewave_gateway_details["secret_key"], "bank_code" => ["110072"]);
                            ...
                            if ($get_beewave_reserve_account["status"] == "success") {
                                ...
                                foreach ($beewave_reserve_account_response["virtual_accounts"] as $beewave_accounts_json) {
                                    $_SESSION["user_session"] = $get_user_detail["username"];
                                    addUserVirtualBank( $beewave_accounts_json["tracking_ref"], $beewave_accounts_json["bank_code"], $beewave_accounts_json["bank_name"], $beewave_accounts_json["account_number"], $beewave_accounts_json["account_name"], $virtual_bank_code_arr[$beewave_accounts_json["bank_code"]], "payvessel.com", "active");
                        ...
                        //Payvessel
                        $select_payvessel_gateway = mysqli_query($connection_server, "SELECT * FROM sas_payment_gateways WHERE vendor_id = '" . $select_vendor_table["id"] . "' AND gateway_name='payvessel' AND `status`='1' LIMIT 1");
                        if (mysqli_num_rows($select_payvessel_gateway) == 1) {
                            ...
                            if ($get_payvessel_access_token["status"] == "success") {
                                ...
                                $payvessel_create_reserve_account_array = array("email" => $user_payvessel_account_reference, "name" => trim($get_user_detail["firstname"] . " " . $get_user_detail["lastname"]), "phoneNumber" => $get_user_detail["phone_number"], "businessid" => $get_payvessel_gateway_details["encrypt_key"], "bankcode" => ["101", "120001"], "account_type" => "STATIC");
                                if (strlen($user_bvn) === 11) {
                                ...
                                if ($get_payvessel_reserve_account["status"] == "success") {
                                    ...
                                    foreach ($payvessel_reserve_account_response["banks"] as $payvessel_accounts_json) {
                                        $_SESSION["user_session"] = $get_user_detail["username"];
                                        addUserVirtualBank($payvessel_accounts_json["trackingReference"], $payvessel_accounts_json["bankCode"], $payvessel_accounts_json["bankName"], $payvessel_accounts_json["accountNumber"], $payvessel_accounts_json["accountName"], $virtual_bank_code_arr[$payvessel_accounts_json["bankCode"]], "payvessel.com", "active");
                                        $account_generation_response = "success";
                                        array_push($account_generation_response, $account_generated_array);
                                    }
                                }
                        ...
                        //Monnify
                        $select_monnify_gateway = mysqli_query($connection_server, "SELECT * FROM sas_payment_gateways WHERE vendor_id = '" . $select_vendor_table["id"] . "' AND gateway_name='monnify' AND `status`='1' LIMIT 1");
                        if (mysqli_num_rows($select_monnify_gateway) == 1) {
                            ...
                            if ($get_monnify_access_token["status"] == "success") {
                                ...
                                fwrite(fopen("validate-user.txt", "a"), "Monnify: " . json_encode($get_monnify_reserve_account) . "\n\n");
                                ...
                                if ($get_monnify_reserve_account["status"] == "success") {
                                    ...
                                    foreach ($monnify_reserve_account_response["responseBody"]["accounts"] as $monnify_accounts_json) {
                                        if (in_array($monnify_accounts_json["bankCode"], array("232", "035", "50515", "058"))) {
                                            $_SESSION["user_session"] = $get_user_detail["username"];
                                            addUserVirtualBank($user_monnify_account_reference, $monnify_accounts_json["bankCode"], $monnify_accounts_json["bankName"], $monnify_accounts_json["accountNumber"], $monnify_reserve_account_response["responseBody"]["accountName"], $virtual_bank_code_arr[$monnify_accounts_json["bankCode"]], "payvessel.com", "active");
                                            $account_generation_response = "success";
                                            array_push($account_generation_response, $account_generated_array);
                                        }
                                    }
