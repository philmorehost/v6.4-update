...
if (isset($_POST["install-product"])) {
    ...
    install_product($connection_server, $get_logged_admin_details, 'betting', 'sas_betting_status', $products_array);
}
...
if (isset($_POST["bulk-update-discount"])) {
    ...
    if (is_numeric($discount_value) && !empty($target_table)) {
        $p_names = "'msport', 'naijabet', 'nairabet', 'bet9ja-agent', 'betland', 'betlion', 'supabet', 'bet9ja', 'bangbet', 'betking', '1xbet', 'betway', 'merrybet', 'mlotto', 'western-lotto', 'hallabet', 'green-lotto'";
                ...
                <form method="post" action="">
                  <table style="" class="table table-responsive table-striped table-bordered" title="Horizontal Scroll: Shift + Mouse Scroll Button">
                    ...
                    foreach ($item_name_array_2 as $products) {
                        $get_item_status_details = mysqli_query_and_fetch_array($connection_server, "SELECT * FROM sas_betting_status WHERE vendor_id='" . $get_logged_admin_details["id"] . "' && product_name='$products'");
