...
ini_set("display_errors", 1); session_start();
            ...
            if(!empty($product_name)){
                if(in_array($product_name, $products_array)){
                    if(is_numeric($item_status) && in_array($item_status, array("0", "1"))){
                        $select_api_lists = mysqli_query($connection_server, "SELECT * FROM sas_apis WHERE vendor_id='".$get_logged_admin_details["id"]."' && id='$api_id' && api_type='nairacard'");
                        $select_nairacard_status_lists = mysqli_query($connection_server, "SELECT * FROM sas_nairacard_status WHERE vendor_id='".$get_logged_admin_details["id"]."' && product_name='$product_name'");
                        if(mysqli_num_rows($select_api_lists) == 1){    
                            if(mysqli_num_rows($select_nairacard_status_lists) == 0){
                                mysqli_query($connection_server, "INSERT INTO sas_nairacard_status (vendor_id, api_id, product_name, status) VALUES ('".$get_logged_admin_details["id"]."', '$api_id', '$product_name', '$item_status')");
                            }else{
                                mysqli_query($connection_server, "UPDATE sas_nairacard_status SET api_id='$api_id', status='$item_status' WHERE vendor_id='".$get_logged_admin_details["id"]."' && product_name='$product_name'");
                            }
                            ...
                            foreach($account_level_table_name_arrays as $account_level_table_name){ 
                                ...
                                if(mysqli_num_rows($select_product_details) == 1){
                                    ...
                                    $select_all_api_lists = mysqli_query($connection_server, "SELECT * FROM sas_apis WHERE vendor_id='".$get_logged_admin_details["id"]."' && api_type='nairacard'");
                        ...
                        //Invalid NAIRACARD Status
                        $json_response_array = array("desc" => "Invalid nairacard Status");
                        $json_response_encode = json_encode($json_response_array,true);
                    }
                }else{
    ...
    if (count($currency_array) == count($credit_amount_array) && count($credit_amount_array) == count($debit_amount_array)) {
        foreach ($currency_array as $index => $api_id) {
            ...
            $dollar_exchange_rate_table = mysqli_query($connection_server, "SELECT * FROM sas_dollar_exchange_rates WHERE vendor_id='" . $get_logged_admin_details["id"] . "' && product_type='nairacard' && currency='" . $currency . "'");
            if (mysqli_num_rows($dollar_exchange_rate_table) == 1) {
                while ($dollar_exchange_rate_details = mysqli_fetch_assoc($dollar_exchange_rate_table)) {
                    mysqli_query($connection_server, "UPDATE sas_dollar_exchange_rates SET credit_amount='" . $credit_amount . "', debit_amount='" . $debit_amount . "' WHERE vendor_id='" . $get_logged_admin_details["id"] . "' && product_type='nairacard' && currency='" . $dollar_exchange_rate_details["currency"] . "'");
                }
            }
        }
...
<body>
    ...
    <section class="section dashboard">
      <div class="col-12">
        ...
        <div class="card info-card px-5 py-5">
                <div class="row mb-3">
                    <span style="user-select: auto;" class="h4 fw-bold">CURRENT NAIRA EXCHANGE</span><br>
                    <div style="user-select: auto; cursor: grab;" class="overflow-auto mt-1">
                        <form method="post" action="">
                                ...
                                <tbody>
                                    ...
                                    foreach ($each_country_exchange_rate_array as $currency) {
                                        ...
                                        if ((mysqli_num_rows($dollar_exchange_rate_table) == 0)) {
                                            mysqli_query($connection_server, "INSERT INTO sas_dollar_exchange_rates (vendor_id, product_type, currency, credit_amount, debit_amount) VALUES ('" . $get_logged_admin_details["id"] . "', 'nairacard', '" . $currency . "', '0', '0')");
                                        }
                                    }
                    ...
                    <tbody>
                        ...
                        <?php
                            ...
                            foreach($item_name_array_2 as $products){
                                $get_item_status_details = mysqli_query_and_fetch_array($connection_server, "SELECT * FROM sas_nairacard_status WHERE vendor_id='".$get_logged_admin_details["id"]."' && product_name='$products'");
